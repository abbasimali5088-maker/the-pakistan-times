...
export async function getArticle(idOrSlug: string) {
  try {
    return await publicFetch<PublicArticle>(`/articles/${encodeURIComponent(idOrSlug)}`, {
      fallback: undefined,
    });
  } catch {
    return null;
...
export async function getTags(params: { page?: number; q?: string } = {}) {
  const data = await publicFetch<Paginated<PublicTag> | { items: PublicTag[] }>(
    `/tags${qs({ public: 1, ...params })}`,
    { fallback: emptyPage<PublicTag>() },
  );
  return {
    items: (data as Paginated<PublicTag>).items || [],
    total: (data as Paginated<PublicTag>).total || 0,
    page: (data as Paginated<PublicTag>).page || 1,
    pageSize: (data as Paginated<PublicTag>).pageSize || 20,
    totalPages: (data as Paginated<PublicTag>).totalPages || 0,
  };
}
...
export async function getMenus(location = "main") {
  ...
  if (data && "items" in data && Array.isArray((data as { items: unknown }).items)) {
    ...
    // shape: { items: MenuItem[] } single menu
    return {
      id: "main",
      name: "Main",
      location,
...
export async function getAd(unitKey: string) {
  try {
    return await publicFetch<AdUnit | { items: AdUnit[] }>(
      `/ads${qs({ unitKey, public: 1 })}`,
      { fallback: null as unknown as AdUnit },
    );
  } catch {
    return null;
  }
...
export async function postNewsletter(payload: { email: string; name?: string }) {
  return publicFetch(`/newsletter`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
    fallback: { ok: false },
...
export async function postNotFound(payload: { url: string; referrer?: string }) {
