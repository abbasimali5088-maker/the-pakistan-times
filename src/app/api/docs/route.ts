...
export async function GET() {
  return handleApi(async () => {
    const paths = [
      ...
      { path: "/api/rates", methods: ["GET", "PUT", "POST"], auth: "optional public=1 / admin sync", notes: "Gold, petrol, diesel, forex — POST ?action=sync fetches live (no paid API key)" },
      { path: "/api/rates/refresh", methods: ["GET", "POST"], auth: "CRON_SECRET", notes: "Vercel Cron every 6h — live sync from free APIs" },
